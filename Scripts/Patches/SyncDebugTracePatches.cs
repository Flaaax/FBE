...
internal static class SyncDebugTrace
{
	...
	internal static string DescribeRewardsSet(RewardsSet? set)
	{
		...
		return
			$"setId={set.Id} owner={set.Player.NetId} rewards=[{string.Join(",", set.Rewards.Select(DescribeReward))}]";
	}
	...
	private static string DescribeReceivedChoices(IEnumerable? choices)
	{
		...
		List<string> items = (from object? choice in choices
			let sender = Field(choice, "senderId")
			let choiceId = Field(choice, "choiceId")
			let task = Field(choice, "completionSource")
			let taskObj = Prop(task, "Task")
			let isCompleted = Prop(taskObj, "IsCompleted")
			select $"sender={sender}:choice={choiceId}:completed={isCompleted}").ToList();
		return string.Join(",", items);
	}
	...
	private static string DescribeEnumerable(object? value)
	{
		...
		List<string> items = (from object? item in enumerable select item?.ToString() ?? "<null>").ToList();
...
internal static class Patch_RewardsSetSynchronizer_CompleteRewardsSet_Trace
{
	...
	private static void Prefix(RewardsSetSynchronizer __instance, object __0, object __1)
	{
		SyncDebugTrace.SafeLog(() =>
		{
			...
			return
				$"CompleteRewardsSet BEFORE state={__1} {SyncDebugTrace.DescribeRewardsSet(set)} {SyncDebugTrace.DescribeRewardSyncState(__instance)}";
		});
	}
	...
	private static void Postfix(RewardsSetSynchronizer __instance, object __0, object __1)
	{
		SyncDebugTrace.SafeLog(() =>
		{
			RewardsSet? set = SyncDebugTrace.Field(__0, "set") as RewardsSet;
			return
				$"CompleteRewardsSet AFTER state={__1} {SyncDebugTrace.DescribeRewardsSet(set)} {SyncDebugTrace.DescribeRewardSyncState(__instance)}";
		});
	}
}
...
internal static class Patch_CardReward_OnSelect_Trace
{
	...
	private static void Prefix(CardReward __instance)
	{
		SyncDebugTrace.SafeLog(() =>
		{
			...
			return
				$"CardReward.OnSelect BEFORE {SyncDebugTrace.DescribeReward(__instance)} cards=[{DescribeCards(cards as IEnumerable)}]";
		});
	}
	...
	private static string DescribeCards(IEnumerable? cards)
	{
		...
		{
			object? card = SyncDebugTrace.Field(cardCreationResult, "Card") ?? AccessTools
				.Property(cardCreationResult?.GetType(), "Card")?.GetValue(cardCreationResult);
